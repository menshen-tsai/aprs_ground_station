...
#include <math.h>
#include <stdio.h>
 // Source: https://github.com/alanbjohnston/Arduino-APRS/blob/master/Arduino-Sketches/Example/APRS_Hello_World/APRS_Hello_World.ino

// Serial1 pins connected to SR105U:
//     UART0 (often called Serial0)
//     TX → GP0
//     RX → GP1
 ...
 *     |<------ tc1200 --------->|<------ tc1200 --------->|
 *     
 * adj_2200 determined the 2200 hz tone adjustment.
 * tc2200 is the half of the 2200 Hz signal periods.
 * 
 *      ------------              ------------              -------
 *     |            |            |            |            |
 *     |            |            |            |            |            
 ...
 * ----              ------------              ------------
 * 
 *     |<--tc2200-->|<--tc2200-->|<--tc2200-->|<--tc2200-->|
 *     
 */
const float baud_adj = 1.00;            //0.975;
const float adj_1200 = 1.0 * baud_adj;
const float adj_2200 = 1.0 * baud_adj;
unsigned int tc1200 = (unsigned int)(0.5 * adj_1200 * 1000000.0 / 1200.0);
unsigned int tc2200 = (unsigned int)(0.5 * adj_2200 * 1000000.0 / 2200.0);

/*
 * This strings will be used to generate AFSK signals, over and over again.
 */
const char *mycall = "BX2XXX";
char myssid = 11;

const char *dest = "APCSS";         // destination callsig of AMSAT CubeSatSim
...
char mystatus[20];
const char *lat = "2504.28N";
const char *lon = "12153.58E";
...
//  Serial1.println("AT+DMOSETGROUP=0,434.9000,434.9000,1,2,1,1\r");
  Serial1.println("AT+DMOSETGROUP=0,435.9000,434.9000,0,0,0,0\r");
//                                  Rx       Tx         
...
void loop()
{
  sprintf(mystatus, "Hello World %2d!!", count++);
  digitalWrite(PTT_PIN, LOW);
  send_packet(_STATUS);
  digitalWrite(PTT_PIN, HIGH);
  
  delay(tx_delay);
  randomize(tx_delay, 5000, 10000);
